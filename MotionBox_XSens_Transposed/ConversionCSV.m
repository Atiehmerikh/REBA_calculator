data = loadbvh('MotionBox_XSens.bvh');
...
for i = 1:length(data) %You run over each line in data. Each line represents one joint
    if isempty(data(i).trans(:,:,1)) ~= 1  %You can only do following code if data.trans contains matrices
        ...
        for j = 1:Nframes %Loop over every frame
            ...
            f_R = fullfile('c:\','Users','jihad','Documents','VUB','Master 1','MA1 Project','matlab','ConversionToCSV','MotionBox_XSens_Transposed',FolderName,'Rotation Matrix',nameR);
            f_T = fullfile('c:\','Users','jihad','Documents','VUB','Master 1','MA1 Project','matlab','ConversionToCSV','MotionBox_XSens_Transposed',FolderName,'Translation Matrix',nameT);

            writematrix(R,f_R);
            writematrix(transpose(T),f_T);
        end
    end  
end
